/**
 * POST /api/admin/gold-price/recalculate-prices - Recalculate prices for ALL products
...
export async function POST(req: Request) {
  try {
    ...
    if (!gold_price_per_gram || typeof gold_price_per_gram !== 'number' || gold_price_per_gram <= 0) {
      return NextResponse.json(
        { error: "Invalid gold_price_per_gram" },
        { status: 400 }
      );
    }
