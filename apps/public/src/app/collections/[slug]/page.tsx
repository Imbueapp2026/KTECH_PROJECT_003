...
export default function CategoryPage() {
  ...
  const [categoryProducts, setCategoryProducts] = useState<ProductJoined[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchCategoryAndProducts = async () => {
      try {
        // Fetch categories to find the matching category ID
        const categoriesResponse = await fetch('/api/categories');
        if (categoriesResponse.ok) {
          const categoriesResult = await categoriesResponse.json();
          
          if (categoriesResult.data) {
            const category = categoriesResult.data.find((cat: { slug: string; name: string; id: string }) => cat.slug === slug);
            if (category) {
              setCategoryName(category.name);
              
              // Fetch products for this category
              const productsResponse = await fetch(`/api/products?category_id=${category.id}&sort=created_at&order=desc&limit=50`);
