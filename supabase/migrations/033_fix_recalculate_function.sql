-- =========================================================
-- Migration 033 — Fix recalculate function to only update published products
-- and allow selective recalculation by material type
-- =========================================================

-- Drop the old function first since the parameter signature is changing (adding default null)
drop function if exists recalculate_all_products_with_missing_data(numeric, numeric);

-- Create new function that handles published products and optionally filters by material_type
...
declare
  ...
  result json;
begin
  -- Process ONLY published products, optionally filtered by material_type
...
comment on function recalculate_all_products_with_missing_data is 'Recalculates prices for published products only, skipping those with missing required data. Can filter by material_type.';
